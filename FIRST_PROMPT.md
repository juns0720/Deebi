...
규칙대로 Current Phase의 docs/phases/NN-*.md 한 개와
Current Task의 docs/tasks/phase-NN/TNN-*.md 한 개만 수행하고,
...
```text
방금 task 테스트 통과했어. docs/PROGRESS.md의 Current Task를 다음 T 번호로 바꾸고,
그 task 하나만 진행해. 끝나면 AWAITING REVIEW로 멈춰.
```

## phase 완료 후 다음 phase 진행

```text
Phase NN의 모든 task 테스트 통과했어. docs/PROGRESS.md의 Current Phase를 다음 phase로 바꾸고,
...
docs/AGENT_GUIDE.md를 지키면서 Current Task 범위 안에서만 수정해.
...
현재 phase/task, 완료된 task, 개발자가 어떤 테스트를 해야 하는지 알려줘.
코드와 문서는 아직 고치지 마.
```
...
Working Notes, Current Phase, Current Task를 기준으로 이어받고,
Current Task가 끝나지 않았으면 마저 끝내고, 끝났으면 멈춘 상태를 유지해.
```
