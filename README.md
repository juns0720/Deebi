...
이 레포는 에이전트 주도 개발 구조다. 개발자는 phase/task 진행과 테스트 승인을 담당하고, 에이전트는 문서 기준으로 설계·구현·검증·문서 갱신을 수행한다.
...
| `docs/phases/00-overview.md` | 전체 phase 목록과 운영 원칙 |
| `docs/tasks/phase-NN/TNN-*.md` | 실제 개발 단위 task 명세 |
| `docs/PROGRESS.md` | 현재 phase/task와 테스트 지시 |
...
1. `docs/PROGRESS.md`에서 Current Phase와 Current Task를 확인한다.
2. 에이전트에게 "현재 task 진행해"라고 말한다.
3. 에이전트가 `Status: AWAITING REVIEW`로 멈추면 `Developer Test`를 직접 확인한다.
4. task 테스트가 통과하면 `PROGRESS.md`의 Current Task를 다음 T 번호로 바꾸고 "다음 task 진행해"라고 말한다.
5. phase 안의 모든 task가 통과했을 때만 Current Phase를 다음 phase로 넘긴다.
6. 실패하면 실패 내용을 알려주고 같은 task 범위 안에서 수정하게 한다.
