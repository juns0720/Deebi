...
- 작업 단위가 끝나면 개발자에게 보고하기 전에 커밋을 만들고 현재 브랜치를 원격에 push한다.
- 커밋 전 `git status`로 변경 범위를 확인하고, 현재 작업과 직접 관련된 파일만 stage한다.
- 원격 push가 실패하면 인증/네트워크/권한 실패 사유를 기록하고 개발자에게 보고한다.
