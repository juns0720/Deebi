...
| 기본 캐릭터 원본 | 개발자 제공 `public/assets/characters/deebi/deebi.png` 보존 |
...
T04의 내 방은 승인된 하나의 응집된 장면을 `rooms/my-room/source/source-scene.png`로 보존한 뒤, 같은 장면을 `rooms/my-room/layers/base-room.png`와 `rooms/my-room/layers/foreground-scene.png`로 분리해 렌더링한다. 이렇게 하면 배경, 책상, 캐릭터가 서로 따로 노는 문제를 줄이고, 두 레이어를 다시 합쳤을 때 승인 원본과 픽셀 단위로 같은 화면을 유지할 수 있다. 중앙 책상 영역 근처에는 러그를 두지 않는다. 개발자 제공 캐릭터 `public/assets/characters/deebi/deebi.png`는 원본을 보존하며, T04의 흰색 DeeBi는 최종 디자인 확정이 아닌 stage 검증용 placeholder로 취급한다.
...
- 중앙 헤더 메뉴 표시용 에셋은 80px 전후 표시 크기에서도 4종의 시각 높이와 픽셀 밀도가 비슷해야 한다.
