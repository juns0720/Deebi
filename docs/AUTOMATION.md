...
이 문서는 개발자가 명령만 해도 에이전트가 다음 작업을 이어갈 수 있게 하는 운영 규칙이다. 실제 단계 상태는 항상 `docs/PROGRESS.md`가 단일 상태원이다.
...
Current Phase 범위 안에서 고치고, 완료 조건을 다시 검증해.
```

### 1.4 상태만 확인

```text
PROGRESS.md를 읽고 지금 무엇이 완료됐고 무엇을 테스트해야 하는지 알려줘.
코드는 고치지 마.
```
...
2. `docs/PROGRESS.md`에서 Current Phase를 확인한다.
3. Current Phase의 `docs/phases/NN-*.md` 하나만 읽는다.
4. 참조 문서 목록을 확인한다.
5. 필요한 문서만 추가로 읽는다.
6. `PROGRESS.md` Working Notes에 계획을 짧게 적는다.
7. 구현한다.
8. phase 완료 조건을 항목별로 검증한다.
...
| 검증 | `docs/QA_CHECKLIST.md` + phase 완료 조건 |
...
- `PROGRESS.md`의 Current Phase 파일이 없다.
- phase 완료 조건이 모호해서 검증할 수 없다.
...
추가 검증은 phase 문서의 완료 조건을 따른다.

## 7. 브라우저 확인 원칙

프론트엔드 화면이 바뀌는 phase에서는 로컬 dev 서버를 띄우고 브라우저에서 확인한다.
