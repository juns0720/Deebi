...
`/dashboard`를 카드형 웹 대시보드가 아니라 공통 게임 클라이언트 프레임처럼 보이게 정리한다. `내 방`과 `함께하는 방`은 `상단 에셋 메뉴 + 중앙 Stage + 우측 선택 메뉴창 + 하단 Dock` 골격을 공유하고, 내 방에서는 헤더의 픽셀 에셋 메뉴를 선택해 상태/꾸미기/뽑기 정보를 확인한다.
...
- 상단에는 중앙 대형 픽셀 에셋 메뉴만 노출한다. 큰 헤더 프레임, 코인 HUD, `DeeBi`/현재 위치, health, 오늘 커밋은 상단에 두지 않는다.
...
- 헤더 에셋 메뉴 동작:
  ...
  - `내 방`: 기존 `상태` 선택 메뉴창을 보여주며 health, streak, owned, sync mock을 보여준다.
  - `꾸미기`: 장착 슬롯과 보유 아이템을 보여준다.
  - `뽑기`: 포인트와 비활성 mock CTA를 보여준다.
- 포인트는 `뽑기` 선택 메뉴창에서만 보인다. `상태` 선택 메뉴창과 상단 헤더에는 별도 포인트/코인 표시를 두지 않는다.
...
- [ ] 상단 헤더에는 브랜드, 현재 위치, health와 오늘 커밋이 보이지 않는다.
- [ ] 상단 헤더 프레임과 코인 HUD 없이 메뉴만 가볍게 보인다.
- [ ] 오늘 커밋은 하단 sync/status 로그 Dock에서 보인다.
- [ ] streak, owned, sync mock, 장착 슬롯, 인벤토리 mock은 우측 선택 메뉴창에서 보인다.
- [ ] 포인트는 뽑기 선택 메뉴창에서만 보인다.
- [ ] 포인트 부족/뽑기 가능 mock 상태가 구분된다.
- [ ] `같이 하기` 에셋 메뉴가 `함께하는 방` 탭 전환으로만 동작한다.
...
3. `내 방`, `꾸미기`, `뽑기` 헤더 에셋 메뉴를 클릭/키보드 포커스로 선택했을 때 우측 선택 메뉴창 내용이 바뀌는지 확인한다.
4. `같이 하기` 에셋 메뉴가 `함께하는 방` 탭 전환으로만 동작하는지 확인한다.
