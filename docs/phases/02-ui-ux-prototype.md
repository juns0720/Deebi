...
Deebi의 핵심 감성인 "내 방에 켜두는 픽셀 캐릭터"와 "친구들과 같은 방에서 개발하는 공동 룸"을 실제 화면으로 먼저 검증한다. 이 단계가 끝나면 DB, OAuth, GitHub API 없이도 랜딩, 내 방, 함께하는 방, 공동 룸, 채팅, 방치 모드를 mock 데이터와 개발용 픽셀 에셋으로 확인할 수 있다.
...
- **해도 됨:** `src/app/**`, `src/components/**`, `src/lib/mock/**`, `src/types/**`, `public/assets/**`, CSS/Tailwind 스타일, UI copy, 접근성/반응형 개선, `docs/DECISIONS.md`, `docs/PROGRESS.md`
...
- P02-T04부터는 `docs/PIXEL_ASSET_PIPELINE.md` 기준으로 개발용 픽셀 에셋을 직접 제작하고, 생성한 에셋을 프로젝트 `public/assets/**`에 저장해 화면에 입힌다.
- 전체 UI를 픽셀 그래픽 감성으로 구현한다. 캐릭터뿐 아니라 버튼, 패널, 탭, 배지, 채팅, 방 오브젝트까지 같은 픽셀 톤을 유지한다.
- Galmuri11을 앱 기본 폰트로 적용한다. 큰 타이틀/HUD 포인트는 Galmuri9를 사용할 수 있다. 폰트 파일이 프로젝트에 없으면 `font-family` 토큰과 fallback을 준비하고, 파일 필요 여부를 Working Notes에 기록한다.
...
- 제품 런타임 AI 이미지 생성은 만들지 않는다. 이미지 생성은 Phase 02 개발 중 정적 에셋 제작으로만 사용한다.

## 완료 조건

- [ ] `/` 랜딩이 Deebi의 핵심 문장과 공동 룸 미리보기를 보여준다.
- [ ] 전체 UI가 픽셀 그래픽 감성을 유지한다.
- [ ] P02-T04 이후 화면은 직접 제작한 방/오브젝트/UI 프레임 에셋을 사용하며, 십자표/개발용 그리드 배경이 남아 있지 않다.
- [ ] Galmuri11이 기본 폰트로 적용되어 있거나, 폰트 파일 부재 시 적용 준비 상태가 명확히 기록되어 있다.
...
5. Galmuri11 또는 준비된 pixel font token이 화면 전체에 적용되는지 확인한다.
