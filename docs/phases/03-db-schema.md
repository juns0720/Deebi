...
Supabase Postgres에 Deebi MVP 데이터 구조를 만들 수 있는 SQL migration과 seed를 작성한다. 이 단계가 끝나면 `users`, `commit_stats`, `items`, `inventory`, `equipped`, `rooms`, `room_members`, `room_messages`가 문서와 일치한다.
...
6. `users.access_token`이 일반 조회 경로로 노출되지 않는지 정책을 확인한다.
